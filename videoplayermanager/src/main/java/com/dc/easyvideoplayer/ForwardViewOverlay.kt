...
/**
 * Overlay for [CustomExoPlayerView] to create a similar UI/UX experience like the official
 * YouTube Android app.
 *
...
class ForwardViewOverlay(context: Context?, private val attrs: AttributeSet?) :
    ...
    companion object {
        const val TAG = ".ForwardViewOverlay"
    ...
    override fun onAttachedToWindow() {
        ...
        if (playerViewRef != -1)
            setPlayerView((this.parent as View).findViewById(playerViewRef) as CustomExoPlayerView)
    }
     ...
     */
    fun setPlayerView(playerView: CustomExoPlayerView) {
    ...
    override fun onDoubleTapProgressUp(posX: Float, posY: Float) {
        if (DEBUG) Log.d(
            TAG, "onDoubleTapProgressUp: $posX")
